 ...
 * Created on 2/27/18
 */
@SuppressWarnings("DuplicatedCode") final class UtilUI {
     ...
     */
    static void sendActionBar(Player player, String message, String... vars) {
        if (player == null)
            return;

        String formattedMessage = format(message, vars);

        clearActionBar(player);

        if (!Bukkit.isPrimaryThread()) {
            UtilThreading.sync(() -> actionBar(player, formattedMessage));
        }

        actionBar(player, formattedMessage);
    }
     ...
     */
    static void clearActionBar(Player player) {
        if (player == null)
            return;
     ...
     */
    static String progressBar(int barSize, double numerator, double denominator, char barChar, ChatColor used, ChatColor free) {
     ...
     */
    static String format(String toFormat, String... variables) {
